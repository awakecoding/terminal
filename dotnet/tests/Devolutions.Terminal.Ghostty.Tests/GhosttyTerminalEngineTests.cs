...
public sealed class GhosttyTerminalEngineTests
{
    ...
    public void FeedProjectsGhosttyGridIntoSharedBuffer()
    {
        ...
        Assert.True(snapshot.Lines[0].Cells[6].Attributes.Flags.HasFlag(Devolutions.Terminal.Core.CellFlags.Bold));
    ...
    public void ProjectionPreservesBlankBackgroundAndWrappedRows()
    {
        ...
        var snapshot = engine.CreateSnapshot().Buffer;
        Assert.Equal(Devolutions.Terminal.Core.ColorKind.Rgb, snapshot.Lines[0].Cells[4].Attributes.Background.Kind);
        Assert.True(snapshot.Lines[0].Wrapped);
        Assert.Equal(coordinateVersion, engine.Buffer.CoordinateVersion);
        Assert.Equal(Devolutions.Terminal.Core.ShellIntegrationKind.None, snapshot.Lines[2].Cells[4].ShellIntegration);
    }
    ...
    {
        using var engine = new GhosttyTerminalEngine(20, 3);
        using var search = new Devolutions.Terminal.TerminalSearchSession(engine);
        engine.Feed("needle\r\ntwo\r\nthree\r\nfour\r\nfive");
    ...
    public void SemanticPromptCreatesOneMarkAtItsActualColumn()
    {
        ...
        Assert.Equal(Devolutions.Terminal.Core.ShellIntegrationKind.Prompt, line.Cells[0].ShellIntegration);
    }
    ...
    {
        using var engine = new GhosttyTerminalEngine(20, 3);
        Devolutions.Terminal.Core.TerminalNotification? notification = null;
    ...
    {
        using var builtIn = new Devolutions.Terminal.Core.TerminalEngine(40, 5);
    ...
    private static string LineText(Devolutions.Terminal.Core.TextBufferSnapshot snapshot, int row) =>
