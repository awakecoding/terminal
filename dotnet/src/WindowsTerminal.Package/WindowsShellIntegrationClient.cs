...
public sealed class WindowsShellIntegrationClient : IWindowsShellIntegrationService
{
        ...
        Func<string, string?>? environment = null)
    {
        ...
        _helperPath = helperPath ?? Path.Combine(AppContext.BaseDirectory, "wt-shell-integration.exe");
    ...
    public ShellIntegrationResult RefreshJumpList(IEnumerable<JumpListProfile> profiles)
    {
        ...
        AddEncoded(request, "executable", Path.Combine(AppContext.BaseDirectory, "WindowsTerminal.exe"));
        ...
        out ShellIntegrationResult unavailable)
    {
        ...
        appId = _environment("WT_DOTNET_AUMID") ?? string.Empty;
        var shortcut = _environment("WT_DOTNET_TOAST_SHORTCUT");
        if (string.IsNullOrWhiteSpace(appId) ||
            (requireShortcut && (string.IsNullOrWhiteSpace(shortcut) || !File.Exists(shortcut))))
        {
            unavailable = ShellIntegrationResult.Unsupported(
                requireShortcut
                    ? "Unpackaged system toasts require WT_DOTNET_AUMID and WT_DOTNET_TOAST_SHORTCUT pointing to a registered Start-menu shortcut."
                    : "Unpackaged jump lists require WT_DOTNET_AUMID for an application with a registered Start-menu shortcut.");
            return false;
        }
