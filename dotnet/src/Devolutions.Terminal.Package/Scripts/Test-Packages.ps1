...
begin {
    ...
    function Test-ShellHelperHashes {
        ...
        foreach ($line in $lines) {
            ...
            $expected = $Matches[1]
            $name = $Matches[2]
            Assert-Condition ($name -in @("Devolutions.Terminal.ShellExt.dll", "dt-shell-integration.exe")) "Unexpected hashed shell helper '$name'."
    ...
    function Test-Msix {
        ...
        try {
            ...
            Assert-Condition (Test-Path -LiteralPath (Join-Path $extractPath "Devolutions.Terminal.exe")) "Devolutions.Terminal.exe is missing from '$Path'."
            ...
            Assert-Condition ($application.Executable -eq "Devolutions.Terminal.exe") "Unexpected application executable."
            ...
            Assert-Condition ($aliases -contains "dt.exe") "The dt.exe execution alias is missing."
            Assert-Condition ($aliases -contains "Devolutions.Terminal.exe") "The Devolutions.Terminal.exe execution alias is missing."

            $protocol = $manifest.SelectSingleNode("//uap3:Protocol[@Name='dterm']", $namespace)
            Assert-Condition ($null -ne $protocol) "The dterm protocol registration is missing."
            ...
            Assert-Condition ($comClass.Path -eq "Devolutions.Terminal.ShellExt.dll") "Unexpected Explorer COM server path."
