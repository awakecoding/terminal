...
`Devolutions.Terminal.Connection` contains a NativeAOT-safe Azure Cloud Shell client built on
...
embedded in `Devolutions.Terminal.Connection`.
...
- Resize updates local dimensions synchronously, coalesces pending service
  ...
  the session.
- `Devolutions.Terminal.App` composes the connection factory, conditionally registers
