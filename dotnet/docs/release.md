...
bash scripts/Test-LinuxArm64Runtime.sh artifacts/packages
```

The test stages an install under `artifacts`, checks desktop/AppStream
...
.\scripts\Test-PublishedApp.ps1 `
  -Executable .\artifacts\native\win-x64\Devolutions.Terminal.exe
```
...
- The development package identity and `dt.exe` alias can conflict with an
